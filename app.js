...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const UI = {
    en: {
      ...
      'exp.eyebrow':'Career','exp.title':'Experience & Education','exp.work':'Work Experience','exp.edu':'Education',
    ...
    fr: {
      ...
      'exp.eyebrow':'Parcours','exp.title':'Expérience & Formation','exp.work':'Expérience Professionnelle','exp.edu':'Formation',
  ...
  $('contact-form').addEventListener('submit', e => {
    e.preventDefault();
    const btn = $('form-btn'), note = $('form-note'), dict = UI[lang];
    btn.disabled = true;
    btn.textContent = dict['form.sending'];
    const name=$('f-name').value, email=$('f-email').value,
          subject=$('f-subject').value||'Portfolio Contact', message=$('f-message').value;
    ...
    setTimeout(() => {
      btn.disabled = false; btn.textContent = dict['form.send'];
      note.textContent = dict['form.sent']; note.className = 'form-note success';
    }, 1200);
  });
