...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const UI = {
    en: {
      ...
      send: 'Send Message', sending: 'Sending…', sent: "Message sent! I'll get back to you soon.", error: 'Something went wrong. Please try again or email me directly.',
    ...
    fr: {
      ...
      send: 'Envoyer le message', sending: 'Envoi…', sent: "Message envoyé ! Je vous répondrai rapidement.", error: "Une erreur est survenue. Réessayez ou contactez-moi directement.",
  ...
  let palette = ls.get('palette') || S.palette || 'blue';
  ...
  function applyPalette(p) {
    palette = p; document.documentElement.setAttribute('data-palette', p); ls.set('palette', p);
    document.querySelectorAll('.palette-dot').forEach((d) => d.classList.toggle('active', d.dataset.palette === p));
  }
  ...
  /* ── NAV LINKS + PALETTE DOTS (built once) ─────────────── */
  function buildNavChrome() {
    ...
    const dots = $('palette-dots'); dots.innerHTML = '';
    PALETTE_KEYS.forEach((p) => {
      const b = el('button', 'palette-dot' + (p === palette ? ' active' : ''));
      b.dataset.palette = p; b.style.background = PALETTE_SWATCH[p];
      b.setAttribute('aria-label', p); b.title = p;
    ...
    const mdots = el('div', 'nav-mobile-dots');
    PALETTE_KEYS.forEach((p) => { const b = el('button', 'palette-dot' + (p === palette ? ' active' : '')); b.dataset.palette = p; b.style.background = PALETTE_SWATCH[p]; b.addEventListener('click', () => applyPalette(p)); mdots.appendChild(b); });
    mob.appendChild(mdots);
  }
  ...
  $('contact-form').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        e.target.reset();
        note.textContent = d.sent; note.className = 'form-note success';
      } else {
        const data = await res.json();
        note.textContent = data?.errors?.map((err) => err.message).join(', ') || d.error;
        note.className = 'form-note error';
      }
    } catch {
      note.textContent = d.error; note.className = 'form-note error';
    }
